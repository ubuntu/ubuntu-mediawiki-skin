...
const Vue = require("vue"),
  { App, restSearchClient, urlGenerator } = require(
...
const searchApiUrl =
  searchConfig.UbuntuTypeahead.apiUrl ||
  mw.config.get("wgScriptPath") + "/rest.php";
const recommendationApiUrl = searchConfig.UbuntuTypeahead.recommendationApiUrl;
const searchOptions = searchConfig.UbuntuTypeahead.options;
...
function initApp(searchBox, restClient, urlGeneratorInstance) {
  ...
  const urlGeneratorConfig = mw.config.get("wgVectorSearchUrlGenerator");
  const searchClientConfig = mw.config.get("wgVectorSearchClient");
  if (urlGeneratorConfig) {
    mw.log
      .warn(`Use of mw.config.get( "wgVectorSearchUrlGenerator") is deprecated.
...
Use SkinPageReadyConfig hook to replace the search module (T395641).`);
  ...
  restClient =
    restClient ||
    searchClientConfig ||
    restSearchClient(searchApiUrl, urlGeneratorInstance, recommendationApiUrl);
  const searchForm = searchBox.querySelector(".cdx-search-input"),
    ...
    searchContainer = searchBox.querySelector(
    ...
    Object.assign(
      {
        ...
        autoExpandWidth: searchBox
          ? searchBox.classList.contains("vector-search-box-auto-expand-width")
          : false,
